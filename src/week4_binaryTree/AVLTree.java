...
public class AVLTree extends BST {
    ...
    /* ----------------- insert ----------------- */
    //노드를 추가할 때마다 imbalance 를 검사해서 바로 교정한다.
    public void insert(char c){
        ...
        while(p!=null){
            ...
            p=p.parent;//balance 가 맞으면 위로 올라간다.
        }
        Node x = p;
        Node y = null;
        //x==null 이라면 root 까지 imbalance 가 없어서 rotate 할 필요가 없다.
        //하지만 x가 null 이 아니라면, imbalance 의 시작점 노드인 것이다.
        if(x!=null){
            if(c<x.key){// c가 x.key 보다 작다는 것은 왼쪽에 있다는 것임. -> LL or LR imbalance 인 경우
                y = x.left;
    ...
    //rotate 로 위로 올라오게 된 노드(x 자리를 차지한 노드)를 반환한다.
    private Node rotateRight(Node x) {//imbalance 가 발생 시작한 노드
        Node y = x.left;
        y.parent = x.parent;
        if (y.parent == null) {//rotate 를 시도한 x가 root 였을 시에
            root = y;
        }
        else{
            if(x==x.parent.left) x.parent.left = y;
...
//    }
    ...
    public Node AVLdelete(char c){
        Node x = delete(c);
        Node y = null; Node z = null; Node w = null;
        while(x!=null){
            if(!isBalanced(x)){
                if(height(x.left)>height(x.right)){//left 의 height 가 더 큰 경우 -> LL 아니면 LR imbalance 가 발생
                    y = x.left;
                    if(y.left!=null){//LL
                        z = y.left;
                        w = rotateRight(x);
                ...
                else{//right 의 height 가 더 큰 경우 -> RR 아니면 RL imbalance 가 발생
                    y = x.right;
                    if (y.left != null) {//RL
                        z = y.left;
                        rotateRight(y);
